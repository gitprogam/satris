...
### 2026-07-08 (추가 완화) 2v2 렉: duo:live를 활성 피스 위치만으로 더 축소 + 직렬화 중복 제거

바로 아래 항목(그리드를 락 때만 보내도록 수정)으로도 사용자가 "완화된 건 맞는데 그래도
너무 느리다"고 재신고. 남은 병목을 더 줄임.

- 변경한 웹 파일:
  1. `server/src/duoRoom.ts` - `duo:live`(매 틱)를 `{active, ghostRow}`만 남기고
     hold/next/score/level/lines/combo/gameOver/pps/apm/garbageQueueLines는 전부
     `duo:board`(락 시에만 오던 메시지)에 `players` 필드로 옮김 - 저 값들은 사실상
     락이 일어날 때만 바뀌므로 매 틱 보낼 이유가 없었음. 추가로 `duo:live` 자체의
     방송 빈도도 60Hz -> 20Hz(`LIVE_BROADCAST_EVERY_N_TICKS=3`)로 낮춤(시뮬레이션은
     그대로 60Hz 정확도 유지, 네트워크 방송만 스로틀). 터널 경유 환경에서는 페이로드
     크기보다 메시지 개수 자체가 지연에 더 크게 기여했음.
  2. `server/src/ws.ts` - `sendAll(sockets, message)` 추가. 기존 `send()`를 여러 소켓에
     반복 호출하면 같은 메시지를 소켓 수만큼 `JSON.stringify`하던 중복이 있었는데,
     한 번만 직렬화해서 재사용하도록 정리. `duoRoom.ts`의 팀 브로드캐스트 지점들을
     전부 이걸로 교체.
  3. `src/net/DuoClient.ts` - `DuoPlayerState`를 `DuoLivePlayerState`(active/ghostRow)
     + `DuoStatsPlayerState`(나머지)로 분리. `DuoBoardMessage`에 `players:
     [DuoStatsPlayerState, DuoStatsPlayerState]` 추가. `DuoStateMessage`(렌더러용 합본
     타입)는 그대로 유지.
  4. `src/duo/DuoSession.ts` - `latestPlayers` 하나였던 걸 `latestLive`/`latestStats`
     둘로 나눠 들고 있다가 렌더링 시점에 스프레드로 합쳐서 넘김.
- 검증: 4소켓 테스트로 `duo:live` 페이로드가 정말 `{active, ghostRow}`만 있는지,
  `duo:board`에 `players`(통계)가 제대로 오는지, 2초간 트래픽이 이전(35KB) 대비 더
  줄었는지(4KB) 확인. 브라우저 4탭 테스트 중 한 번은 재시작 안 한 예전 서버에 접속해서
  화면이 안 뜨는 걸 발견했는데(예전 서버의 `duo:board`엔 `players`가 없어서
  `latestStats`가 계속 `undefined`) - 이건 버전 불일치 때문이었고 서버를 재시작하니
  정상 렌더링됨. **서버 코드(`server/`)를 고칠 때마다 반드시 실행 중인 서버 프로세스를
  재시작해야 반영된다는 점 주의** (tsx watch가 아니라 일반 tsx라 자동 리로드 안 됨).
- 왜: 렉 완화가 부족하다는 재신고.
- Godot 쪽 대응: 해당 없음(PvP 자체가 아직 없음).

### 2026-07-08 2v2 듀오 모드 렉 수정: 매 틱 전체 그리드 전송 제거

사용자가 "2대2 렉이 너무 심하다"고 신고. 원인: 서버가 매 틱(16ms, 60Hz)마다 20x40칸
전체 보드를 JSON으로 통째로 팀 소켓들에 보내고 있었음 - 실제로 보드가 바뀌는 건 락이
일어날 때뿐인데 그와 무관하게 계속 보내고 있었던 게 문제.

- 변경한 웹 파일(서버는 이 저장소 안 `server/`, 같이 기록):
  1. `server/src/duoRoom.ts` - 브로드캐스트를 두 종류로 분리:
     - `duo:live`(매 틱, 가벼움): 활성 피스 위치/홀드/넥스트/점수 등 `playerState()`만,
       그리드 없음.
     - `duo:board`(락이 나서 `GameEngine.onBoardChanged`가 실제로 호출될 때만): 그
       팀의 그리드. 자기 팀 소켓엔 `duo:board`로, 상대 팀 소켓엔 기존처럼
       `duo:enemyBoard`로 즉시 전달(기존의 "5틱마다 스로틀" 방식 제거 - 이제 딜레이
       없이 실제 변경 시점에 바로 감).
     - 매치 시작 직후 양 팀에 `duo:board`를 한 번씩 먼저 보내서, 첫 락 전에도 클라가
       빈 보드를 못 받는 일이 없게 함.
  2. `src/net/DuoClient.ts` - `onState`(그리드+플레이어 합본) 콜백을 `onLive`/`onBoard`
     로 분리. `DuoStateMessage` 타입은 네트워크 메시지가 아니라 클라이언트가 둘을
     합쳐서 렌더러에 넘길 때 쓰는 용도로 남겨둠.
  3. `src/duo/DuoSession.ts` - 최신 그리드(`latestGrid`)와 플레이어 상태(`latestPlayers`)
     를 각각 따로 들고 있다가 렌더링 시점에 합쳐서 `DuoRenderer.render()`에 넘김.
- 검증: 4소켓 테스트로 확인 - 락이 없는 2초 동안 `duo:board`는 0번(합격), `duo:live`만
  꾸준히 옴(2초간 약 35KB, 예전엔 매 틱 전체 그리드라 수백 KB/초 수준이었을 것). 락 1번
  치면 자기 팀엔 `duo:board`, 상대 팀엔 `duo:enemyBoard`가 정확히 옴.
- 왜: 실사용 중 렉 신고.
- Godot 쪽 대응: PvP 자체가 아직 없어서 해당 없음. 나중에 포팅할 때 "그리드는 변경
  시에만, 위치정보는 매 틱" 원칙을 처음부터 반영할 것.
